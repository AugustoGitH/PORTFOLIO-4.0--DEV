...
export default {
  ...
  async getProjects(req: Request, res: Response) {
    try {
      ...
      console.log(
        `Ocorreu um erro ao buscar projetos na rota publica -------> ${error}`
      );
      res.status(500).send({
        message:
          "Ocorreu um erro interno no servidor na tentativa de resgatar projetos.",
      });
    }
  },
  ...
  async likeProject(req: Request, res: Response) {
    ...
    try {
      ...
      await Project.findByIdAndUpdate(idProject, {
        $inc: {
          likes:
            stateLike === FAVORITE ? 1 : stateLike === DESFAVORITE ? -1 : 0,
        },
      });
