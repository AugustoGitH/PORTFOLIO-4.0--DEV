...
import updateProjectService from "../services/Admin/updateProject"
import { queryClient } from "../services/queryClient";

import deleteProjectService from "../services/Admin/deleteProject";
...
const useEditProject = (project: TProject)=>{
  ...
  const [ valuesProject, setValuesProject ] = useState<TProjectEditabled>(originValues)

  const [ comparisons, setComparisons ] = useState<TReturnCompareEditValues| null>(null)
  ...
  const handleUpdateProject = ()=>{
    ...
    updateProjectService({...valuesProject, idProject: project._id}).then(response=>{
      alert(response.message)
      if(response.update){
        setIsLoading(false)
        resetUpdate()
