...
const useProjectsAdminStore = create<TProjectsAdminStore>(set=>({
    ...
    deleteProject: (idProject: string)=>{
      set(props=>({
        projects: props.projects?.filter(project=> project._id !== idProject) || null,
      }))
    },
...
const useProjectsAdmin = ()=>{
  ...
  const { setProjects, dataDashboard, deleteProject, projects, setDataDashboard } = useProjectsAdminStore()
  useEffect(()=>{
    if(projectsData){
  ...
  const updateProject = (idProject: string, fieldsEdit: TProjectEditabled)=>{
    // if(!projects ) throw new Error("Você não pode atualizar um projeto antes da lista de projetos ser carregada.")

    // const projectSelected = projects.find(project=> project._id === idProject)

    // if(!projectSelected) throw new Error("Você não pode atualizar um projeto que nem existe na lista atual de projetos.")

    // const newProjectEdited = updateProjectWithEditedFields(projectSelected, fieldsEdit)

    // setProjects([...projects.filter(project=> project._id !== idProject), newProjectEdited])
